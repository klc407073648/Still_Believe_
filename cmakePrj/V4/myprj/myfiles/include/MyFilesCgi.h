...
public:
         ...
         *
         */
        int getUserFilesCount(const string& user);
                /**
         * @brief  校验用户令牌 
         *
         * @param [in] user      文件上传者
		...
		*/
        void returnLoginStatus(const long& num,const int& tokenFlag);
		void returnStatus(const string& statusNum);
        ...
        * @param [in] token     文件令牌
		*/
        int getUserFileList(const string& cmd, const string& user ,const int& start, const int& count);
