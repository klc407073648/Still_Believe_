...
namespace pubsub
{
typedef std::shared_ptr<PubSubClient> PubSubClientPtr;
...
class Topic : public muduo::copyable
{
 public:
  ...
  void add(const PubSubClientPtr& conn)
  {
  ...
  void remove(const PubSubClientPtr& conn)
  {
  ...
  void publish(const string& content, Timestamp time)
  {
    ...
    for (std::set<PubSubClientPtr>::iterator it = audiences_.begin();
         it != audiences_.end();
         ++it)
    {
      (*it)->send(message);
 ...
 private:
  ...
  string topic_;
  string content_;
  std::set<PubSubClientPtr> audiences_;
};

class PubSubServer : noncopyable
{
 public:
  PubSubServer(const std::string& ip , int port)
    : loop_(loop),
      server_(ip, port, "PubSubServer")
  {
 ...
 private:
  void onConnection(const PubSubClientPtr& client)
  {
    if (client->connected())
    {
  ...
  void onMessage(const PubSubClientPtr& client,
                 Buffer* buf,
