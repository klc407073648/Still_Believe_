...
CREATE_STATIC_LOGGERMANAGER(CMyRedis);

CMyRedis::ptr CMyRedis::m_MyRedis = NULL;
...
static CMyRedis::ptr  CMyRedis::getInstance(const string& ip , int port)
{
	if(m_MyRedis ==NULL)
	{
		...
		m_MyRedis->Connect(ip,port);
		//m_MyRedis->Connect("127.0.0.1",6379);
		//m_MyRedis->Connect(temp["redis_ip"],atoi((temp["redis_port"]).c_str()));
	}
	return m_MyRedis;
}

void CMyRedis::redisTest()
{
	try{
		CMyRedis myRedis;
		myRedis.Connect("127.0.0.1",6379);
		LogInfo("check connect: %d",myRedis.CheckConenct());
		myRedis.Set("hello", "world");
		LogInfo("myRedis.Get hello is %s",myRedis.Get("hello").c_str());
	}
	catch(CErrorMsg& cErr)
	{
		LogError("error code = %d, errmsg=%s\r\n", cErr.get_errorCode(), cErr.get_errorMsg().c_str());
	}
}
...
{
	m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"setex %s %d %s", strKey.c_str(), strSec,strValue.c_str());
	if(m_pRedisReply == NULL)
	{
		freeReplyObject(m_pRedisReply);
		throw CErrorMsg("set error! m_pRedisReply is NULL");
...
int CMyRedis::ZLexCount(const string& strZSetKey, const string& value)
{
	m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"zlexcount %s [%s [%s", strZSetKey.c_str(), value.c_str(), value.c_str());
	if(m_pRedisReply == NULL || m_pRedisReply->type != REDIS_REPLY_INTEGER)
	{
		freeReplyObject(m_pRedisReply);
		throw CErrorMsg("zlexcount error!");
	}
	int nRet = m_pRedisReply->integer;
	freeReplyObject(m_pRedisReply);
	return nRet;
...
int CMyRedis::Zcard(const string& strZSetKey)
{
	m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"ZCARD %s", strZSetKey.c_str());
	if(m_pRedisReply == NULL || m_pRedisReply->type != REDIS_REPLY_INTEGER)
	{
		freeReplyObject(m_pRedisReply);
		throw CErrorMsg("zlexcount error!");
...
int CMyRedis::ZScore(const string& strZSetKey, const string& value)
{
	m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"ZSCORE %s %s", strZSetKey.c_str(), value.c_str());
	if(m_pRedisReply == NULL || m_pRedisReply->type != REDIS_REPLY_INTEGER)
	{
		freeReplyObject(m_pRedisReply);
		throw CErrorMsg("ZScore error!");
	}
	int nRet = m_pRedisReply->integer;
	freeReplyObject(m_pRedisReply);
	return nRet;
...
int CMyRedis::ZIncre(const string& strZSetKey, const string& value)
{
	m_pRedisReply = (redisReply*)redisCommand(m_pRedisContext,"ZINCRBY %s 1 %s", strZSetKey.c_str(), value.c_str());
	if(m_pRedisReply == NULL || m_pRedisReply->type != REDIS_REPLY_STRING)
	{
		freeReplyObject(m_pRedisReply);
		throw CErrorMsg("ZINCRBY error!");
	}
	int nRet = atoi(m_pRedisReply->str);
	freeReplyObject(m_pRedisReply);
	return nRet;
}
