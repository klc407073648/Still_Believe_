...
int Md5Cgi::dealMd5(const string& user , const string& md5, const string& fileName)
{
    ...
    if( mysql->getAffectedRows() != 0) //有结果，说明服务器已经有此文件
    {
        ...
        sql_select = sql_cmd;
        mysql->query(sql_select); 
        if( mysql->getAffectedRows() != 0 ) //如果有结果，说明此用户已经保存此文件
        {
            LogError("%s[filename:%s, md5:%s]已存在\n", user.c_str(), fileName.c_str(), md5.c_str());
        ...
        */
        ...
        string mysql_str = sql_cmd;
        mysql->query(mysql_str);
...
*/
void Md5Cgi::returnMd5Status(const string& status_num)
{
	Json::Value root;
